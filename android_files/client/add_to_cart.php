...
if ($_SERVER['REQUEST_METHOD'] =='POST'){
    ...
    if($row['stock']< $qty){
        ...
        $response["status"] = 0;
        $response["message"] = 'Sorry, the pieces remaining are only  '.$row['stock'];
...
// check if item exist in client shopping cart
        ...
        $select= "SELECT * FROM order_items WHERE client_id='$clientID' AND stock_id='$productID' AND item_status='0' AND type = 'order'";
        ...
        if($get=mysqli_num_rows($records) >0){
            ...
            if ($row=mysqli_num_rows($record) < 1 ){
                ...
                $insrt= "INSERT INTO order_items (client_id, order_id, stock_id, quantity,type)
             VALUES ('$clientID','$orderNo', '$productID', '$qty','order')";
                ...
                $insert= "INSERT INTO order_items (client_id, order_id, stock_id,quantity,type)
                  VALUES ('$clientID', '$orderNo', '$productID', '$qty','order')";
